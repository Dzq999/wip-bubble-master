...
description: WIP Bubble SOP 预置的总控与路由 Skill。用于用户询问 WIP Bubble、高 WIP、WIP 最高原因，或确认是否进入下一流程时；负责创建 UUID case_id、读取 vfab_agent 流程记录、严格按 Flow 顺序推进、加载通用工厂异常处理知识、调用内部流程模块，并返回 text/json/both。
---

# WIP Bubble 总控 Skill

这是FaaS 平台唯一需要调用的外层 Skill。用户不需要提供 `case_id`；本 Skill 会自动创建 UUID 形式的 `case_id`，并写入 `vfab_agent.fab_case_flow_record`。
...
- 如果需要本地调试脚本，可读取脚本返回的 `model_context.raw_inputs` 和 prompt；面向用户时必须输出当前路由 Flow 的业务文本与结构化 JSON 结构。
...
5. 对外 `text` 与 `content` 禁止出现实现、展示或测试来源的术语，也不得把规则来源表述为业务依据。

## 渐进式披露

执行本 Skill 时必须遵守 [progressive-disclosure.md](references/progressive-disclosure.md)：总控先完成路由，再按需读取当前目标子 Skill 的资源；禁止预加载兄弟 Flow、所有示例或全部知识库。后续 Flow 只通过前序 `content` / `text` 与 `case_data_snapshot.sql_results` 获取事实。

## 生成、落库与最终返回顺序

正式执行任一 Flow 时，当前 Agent 必须把“本轮新生成的完整 Flow 输出”作为唯一最终答案来源。禁止为了节省步骤，直接读取目标 Flow 已存在的 `flow_data_json` 并返回给用户或展示层。
...
当总控脚本返回 `save_required=true`、`must_generate_before_final=true`、`must_save_before_final=true` 或 `must_not_return_existing_flow_data=true` 时，上述顺序是硬约束。只有明确进行本地格式调试且用户要求不落库时，才可以跳过保存；既定业务字段 / FaaS 正式链路默认必须先保存再返回。
## 离线数据模式
...
这份知识描述真实工厂异常处理闭环：异常发现、异常确认、异常分级、影响评估、原因候选、跨部门协同处置、工程师二次分析、恢复验证和案例沉淀。
...
- `examples/master/start-request.json`
- `examples/master/start-response.json`

Flow 专属模板保存在对应子 Skill 的 `examples/` 目录：

```text
internal/wip-flow-01-anomaly-detection/examples/
...
internal/wip-flow-11-retrospective/examples/
```

总控不得枚举、预加载或读取所有 Flow 模板。只有在维护或核对当前 Flow 的结构时，才按需读取当前子 Skill 的 `examples/`；示例不参与运行，也不是业务事实来源。

## 展示层输出约定

每个 Flow 执行完成后，都必须输出既定业务字段 需要的三块主容器；不要再输出重复的 `internal_payload`、`internal_render` 或顶层分散容器。
...
`content.containers` 的一级标题固定，二级/三级内容由对应 Flow 的 output-contracts 约束；展示层只需要循环 `containers[].sections[].items[]` 动态渲染。
...
`return_type=text` 是默认值，只输出面向用户阅读的 Markdown 文本，不附加 JSON。该 Markdown 必须按既定业务字段 的三个主容器组织：
...
其中第一个区块供用户直接阅读；第二个区块供展示层/编排消费。虽然完整 JSON 内仍包含 `text` 字段，但不得因此省略第一个可读 Markdown 区块。
## 当前 Agent 执行流程
...
4. 最终面向用户或对外展示的 `text` / `content` 禁止出现 `mock` 字样；来自内部 mock 的字段必须按业务口径表述为“快照输入”“补充信号”或具体数据名称。
...
9. 每个 Flow 都维护自己的 `knowledge/` 和 `prompts/`：
   ...
   - Flow 内部 `knowledge/` 存放本 Flow 的业务知识、判断条件、角色口径和 输出约束。
   - prompts/ 存放模型生成提示词。
   - 每个内部 Flow 子 skill 都应维护 output-contracts/，单独存放最终回答格式契约；契约文件要拆分说明 Markdown 文本结构和轻量 JSON 结构。
   - Python 脚本只负责 Case 启动时的一次性 SQL 快照、读取保留的补充数据、执行模型显式要求的确定性计算、解析/修复模型 JSON 和保存模型结果；不要在脚本里硬编码最终文本话术、展示容器内容或业务分析结论。
