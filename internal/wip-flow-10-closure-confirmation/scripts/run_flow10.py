#!/usr/bin/env python
"""Flow 10: prepare raw inputs for agent-generated case closure confirmation output."""
...
PROMPT_PATH = Path(__file__).resolve().parents[1] / "prompts" / "flow10_result_prompt.md"
OUTPUT_CONTRACT_DIR = Path(__file__).resolve().parents[1] / "output-contracts"
TEXT_OUTPUT_CONTRACT_PATH = OUTPUT_CONTRACT_DIR / "flow10-text-output-contract.md"
JSON_OUTPUT_CONTRACT_PATH = OUTPUT_CONTRACT_DIR / "flow10-json-output-contract.md"
GLOBAL_KNOWLEDGE_DIR = SKILL_DIR / "knowledge"
FLOW_KNOWLEDGE_DIR = Path(__file__).resolve().parents[1] / "knowledge"
FLOW_MOCK_PATH = Path(__file__).resolve().parents[1] / "data" / "flow10_mock.json"
...
def extract_flow09_closure_context(previous_flow_contents: list[Dict[str, Any]]) -> Dict[str, Any]:
    result: Dict[str, Any] = {"closure_inputs": []}
    for summary in reversed(previous_flow_contents):
        if str(summary.get("flow_no") or "") != "09":
            continue
        ...
        for item in iter_content_items(content):
            ...
            if lower_label in {"impact clearance status", "observation result", "recurrence risk", "downstream transfer risk", "side effect", "control release readiness", "next flow"}:
                result["closure_inputs"].append({"label": label, "value": value, "meta": item.get("meta")})
            elif "impact clearance status" in lower_label:
                result["impact_clearance_status"] = value
            elif "observation result" in lower_label:
                result["observation_result"] = value
            elif "control release readiness" in lower_label:
                result["control_release_readiness"] = value
        result["source_flow_no"] = summary.get("flow_no")
        break
    return _drop_empty(result)


def build_flow10_inputs(previous_flow_contents: list[Dict[str, Any]], case_data_snapshot: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        "flow10_inputs": load_flow_mock(),
        "case_data_snapshot": snapshot,
        "sql_results": sql_results,
        "derived_context": {
            "flow09_closure_context": extract_flow09_closure_context(previous_flow_contents),
        },
    }
...
def build_model_context(case_id: str, previous_record: Optional[Any] = None) -> Dict[str, Any]:
    ...
    flow10_context = build_flow10_inputs(previous_flow_contents, case_data_snapshot=case_data_snapshot)
    return {
        "case_id": case_id,
        "flow": {"flow_no": FLOW_NO, "flow_name": FLOW_NAME, "purpose": "Case 关闭确认"},
        ...
        "generation_rules": [
            ...
            "同时使用 case_data_snapshot.sql_results 中的原始异常事实与 Flow 08 之后 content 中的恢复趋势；不要读取或返回旧 Flow 10 结果，也不得引用 examples 的示例数值。",
            "Flow 10 默认假设关闭确认通过，因为 Flow 09 已确认影响消除观察通过。",
            "Flow 10 只确认任务完成、风险解除、指标恢复、受控动作关闭和责任人关闭确认；不要做复盘沉淀。",
            "WIP Case Snapshot 仍然只包含两段：Case Header 和 Case Closure Checklist｜关闭确认。",
            "正常演示输出 closure_confirmation_status=关闭确认通过、case_status=Processing、next_flow_no=11、next_flow_name=Case 复盘沉淀。",
            "脚本不构造最终展示结构或固定话术。",
        ],
...
REQUIRED_CONTENT_SHAPE = [
    ("WIP Case Snapshot", ["Case Header", "Case Closure Checklist｜关闭确认"]),
    (
        "当前阶段对话",
...
def validate_data_tool_call_items(content: Dict[str, Any]) -> None:
    ...
    if missing:
        raise ValueError("Flow 10 data/tool calls must include closure-confirmation dimensions: " + ", ".join(missing))
...
def normalize_model_output(model_output: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "closure_confirmation_status": model_output.get("closure_confirmation_status"),
    }


def decision_from_model_output(model_output: Dict[str, Any]) -> Dict[str, Any]:
    status = str(model_output.get("closure_confirmation_status") or "")
    combined = json.dumps(model_output, ensure_ascii=False)
    blocked_terms = ("证据不足", "未通过", "未确认", "未完成", "未解除", "未恢复", "未释放", "需回退", "升级", "On Hold", "on hold")
    if any(term in status for term in blocked_terms) and "关闭确认通过" not in status:
        default = {"flow_status": "Closed", "case_status": "On Hold", "next_flow_no": None, "next_flow_name": None}
    elif any(term in combined for term in ("任务未完成", "风险未解除", "指标未恢复", "受控动作未关闭", "责任人未确认")):
        default = {"flow_status": "Closed", "case_status": "On Hold", "next_flow_no": None, "next_flow_name": None}
    else:
...
def compose_flow09_result(case_id: str, previous_record: Optional[Any] = None, model_output: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "closure_confirmation_status": generated.get("closure_confirmation_status"),
        "text": generated["text"],
...
def run(case_id: str, previous_record: Optional[Any] = None, model_output: Optional[Dict[str, Any]] = None, save: bool = True) -> Dict[str, Any]:
    result = compose_flow09_result(case_id, previous_record=previous_record, model_output=model_output)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run WIP Bubble Flow 10.")
