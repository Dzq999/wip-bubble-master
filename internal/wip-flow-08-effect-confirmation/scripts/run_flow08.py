#!/usr/bin/env python
"""Flow 08: prepare raw inputs for agent-generated effect confirmation output."""
...
PROMPT_PATH = Path(__file__).resolve().parents[1] / "prompts" / "flow08_result_prompt.md"
OUTPUT_CONTRACT_DIR = Path(__file__).resolve().parents[1] / "output-contracts"
TEXT_OUTPUT_CONTRACT_PATH = OUTPUT_CONTRACT_DIR / "flow08-text-output-contract.md"
JSON_OUTPUT_CONTRACT_PATH = OUTPUT_CONTRACT_DIR / "flow08-json-output-contract.md"
...
def extract_flow07_recovery_context(previous_flow_contents: list[Dict[str, Any]]) -> Dict[str, Any]:
    result: Dict[str, Any] = {}
    for summary in reversed(previous_flow_contents):
        if str(summary.get("flow_no") or "") != "07":
            continue
        for item in iter_content_items(summary.get("content", {})):
            ...
            if "collaboration package status" in lower_label:
                result["collaboration_package_status"] = value
            elif "primary owner" in lower_label:
                result["primary_owner"] = value
            elif "collaboration tasks" in lower_label:
                result["collaboration_tasks"] = value
            elif "sla" in lower_label:
                result["sla"] = value
            elif "recovery metrics" in lower_label or "recovery metric" in lower_label:
                result["recovery_metrics"] = value
            elif "package scope" in lower_label:
                result["package_scope"] = value
        if result:
...
def validate_data_tool_call_items(content: Dict[str, Any]) -> None:
    ...
    if missing:
        raise ValueError("Flow 08 data/tool calls must include effect-confirmation dimensions: " + ", ".join(missing))
...
def normalize_model_output(model_output: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "effect_confirmation_status": model_output.get("effect_confirmation_status"),
    }


def decision_from_model_output(model_output: Dict[str, Any]) -> Dict[str, Any]:
    status = str(model_output.get("effect_confirmation_status") or "")
    blocked_terms = ("证据不足", "未确认", "需回退", "升级", "On Hold", "on hold")
...
def compose_flow08_result(case_id: str, previous_record: Optional[Any] = None, model_output: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "effect_confirmation_status": generated.get("effect_confirmation_status"),
        "text": generated["text"],
...
def run(case_id: str, previous_record: Optional[Any] = None, model_output: Optional[Dict[str, Any]] = None, save: bool = True) -> Dict[str, Any]:
    result = compose_flow08_result(case_id, previous_record=previous_record, model_output=model_output)
