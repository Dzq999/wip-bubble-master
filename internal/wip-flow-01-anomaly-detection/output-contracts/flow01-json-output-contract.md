...
本文件只规定 Flow 01 给展示层或编排层使用的轻量结构化 JSON。最终可见内容禁止出现 `mock` 字样；来自内部 mock 的字段也必须按业务口径表述为快照输入、补充信号或具体数据名称。禁止只输出摘要 JSON；`json` 或 `both` 中的 JSON 必须包含完整 `text` 和完整 `content.containers`。

## 设计目标

JSON 只需要覆盖 Markdown 文本内容，方便展示层按容器循环渲染；不要重复维护一套复杂的展示层专用结构。

`content` 不是字段拼盘。每个 section 的 `items` 必须与 Markdown 同一段分析对齐，体现本次数据从触发、观察、判断到门禁的上下文关系。

## 未发现异常的轻量关闭结果

当未发现达到 WIP Bubble 条件的 Stage 时，`text` 固定为：`未发现达到 WIP Bubble 条件的业务 Stage，本次 Flow 01 已关闭，不进入 Flow 02。`。

此时仍保留 `content.containers` 的三个一级容器，但每个容器只保留必要的单个 section：

1. `WIP Case Snapshot`：`Case Header`，仅含 Case ID 与 Closed 状态。
2. `当前阶段对话`：`检查结论`，说明未发现异常 Stage。
3. `当前阶段结果`：`本阶段结论`，说明本次关闭且不进入 Flow 02。

轻量关闭结果不要求完整风险快照、9 个当前阶段对话子段落或 5 个当前阶段结果子段落；其他结果仍按下述完整结构输出。

必须避免：

- 不要输出 `internal_render`。
- 不要输出 `internal_payload`。
...
异常成立或待处理时，`content.containers[0]` 必须直接使用 `wip-case-snapshot` 返回的 `wip_case_snapshot`。Flow 01 不得改写该容器的字段、数值或顺序；字段清单由 `wip-case-snapshot/output-contracts/snapshot-display-schema.md` 控制。
...
### Case Header 的 Occur Time

`Case Header` 中 `label=Occur Time` 的 `value` 必须取 `case_data_snapshot.sql_results.locate_high_wip_stage.update_time`。不得使用 `biz_time`、快照采集时间、当前系统时间或补充数据中的时间；没有 `update_time` 时省略该 item。
...
每个容器下使用 `sections` 表示三级标题内容，展示层直接循环渲染。最终 JSON 必须一次性输出 3 个容器。异常成立或需进一步处理时，缺少任意规定 section 都是不合格结果；未发现异常时按“未发现异常的轻量关闭结果”执行。
  ...
  "sections": [
    {
      ...
      "items": [
        ...
        {"label": "Occur Time", "value": "<locate_high_wip_stage.update_time>"},
...
Downstream 必须来自 `case_data_snapshot.sql_results.locate_downstream_starvation` 的 SQL 原始字段：`next_stage_name`、`actual_wip`、`target_wip`、`wip_ratio`、`if_starved`；不要把 stage 名和状态拼成固定短语；`Clean` 只是示例 stage name，真实值必须来自 SQL。SQL 和 mock 都没有的数据不要输出占位项。

## JSON 输出前自检

最终 JSON 必须确认：

- 顶层没有 `internal_render`。
- 顶层没有 `internal_payload`。
