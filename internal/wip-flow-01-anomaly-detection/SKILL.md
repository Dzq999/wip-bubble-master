...
description: WIP Bubble SOP 的 Flow 01 内部模块。仅用于第一个流程“异常发现”：查询或接收 WIP stage 原始数据，读取必要 mock 和知识库，由当前 Agent 生成前端 JSON 与人工可读文本，并用 Python 做 JSON 修复和结果保存。
---
...
- 不在 Python 中硬编码最终文本话术、前端容器内容或业务分析结论。

分析表达、前端结构化内容和用户可读 Markdown 应由当前 Agent 结合 `model_context.raw_inputs` 与知识库生成。示例输入输出只作为结构参考，正文表述不能原文照抄。
...
```text
examples/flow-01/model-context-input.json
examples/flow-01/model-output.json
```

这两个文件用于说明 Flow 01 输入上下文和输出结构，不参与运行。其中 `examples/flow-01/model-output.json` 同时对齐 `fab-agent-demo` 前端输入结构，包含 轻量 `content.containers[].sections[].items[]`，不要把它当作普通 mock 文本清理。
...
```json
{
  ...
  "case_snapshot": "Flow 执行前生成的快照，可选",
...
执行 Flow 01 前，总控 Skill 会先调用 `wip-case-snapshot` 生成原始 `case_snapshot` 输入包；Flow 01 Agent 根据该输入包生成最终 WIP Case Snapshot。
...
Flow 01 结果通过轻量 `content.containers` 表达三块前端容器：
...
当前 Agent 应依据 `model_context.raw_inputs`、Prompt 和知识库自行生成内容；Python 不负责补齐前端容器，也不复制示例正文。
...
```bash
python .agents/skills/wip-bubble-master/internal/wip-flow-01-anomaly-detection/scripts/run_flow01.py --case-id <uuid> --model-output-json @examples/flow-01/model-output.json --validate-only
python .agents/skills/wip-bubble-master/internal/wip-flow-01-anomaly-detection/scripts/run_flow01.py --case-id <uuid> --model-output-json @examples/flow-01/model-output.json --return-type json
```
