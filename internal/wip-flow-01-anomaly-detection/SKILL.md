...
执行 Flow 01 前，总控 Skill 会通过 `wip-data-query.collect_case_data_snapshot` 一次性查询白名单 SQL，并基于其中的 `flow01.warehouse_high_wip`、`flow01.downstream_starvation` 生成原始 `case_snapshot` 输入包；Flow 01 Agent 根据该输入包生成最终 WIP Case Snapshot。独立调试 Flow 01 脚本时，脚本也会使用同一统一快照入口，不能构造空 `model_context`。
