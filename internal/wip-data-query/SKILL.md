...
本模块只在 Flow 01 启动、离线快照加载或明确的数据访问任务中按需读取。不要预加载其他 Flow 的资源；详细规则见 [渐进式披露规则](../../references/progressive-disclosure.md)。
...
- `collect_case_data_snapshot`：在 Case 启动时一次性执行当前 当前配置已配置的白名单 SQL，返回 `schema_version=case-data-snapshot-v1` 的快照。快照会保存到 Flow 01 的 `flow_data_json.case_data_snapshot`，供 Flow 03 / Flow 04 / Flow 06 及后续流程复用。
...
- 查询脚本只返回 SQL 原始结果快照或写入模型已经生成的结果，不负责字段展示编排、文本话术、展示容器或业务结论构造。
