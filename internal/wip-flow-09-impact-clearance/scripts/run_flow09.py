#!/usr/bin/env python
"""Flow 09: prepare raw inputs for agent-generated impact clearance observation output."""
...
PROMPT_PATH = Path(__file__).resolve().parents[1] / "prompts" / "flow09_result_prompt.md"
OUTPUT_CONTRACT_DIR = Path(__file__).resolve().parents[1] / "output-contracts"
TEXT_OUTPUT_CONTRACT_PATH = OUTPUT_CONTRACT_DIR / "flow09-text-output-contract.md"
JSON_OUTPUT_CONTRACT_PATH = OUTPUT_CONTRACT_DIR / "flow09-json-output-contract.md"
GLOBAL_KNOWLEDGE_DIR = SKILL_DIR / "knowledge"
FLOW_KNOWLEDGE_DIR = Path(__file__).resolve().parents[1] / "knowledge"
FLOW_MOCK_PATH = Path(__file__).resolve().parents[1] / "data" / "flow09_mock.json"
...
def extract_flow08_observation_context(previous_flow_contents: list[Dict[str, Any]]) -> Dict[str, Any]:
    result: Dict[str, Any] = {"trend_items": []}
    for summary in reversed(previous_flow_contents):
        if str(summary.get("flow_no") or "") != "08":
            continue
        ...
        for item in iter_content_items(content):
            ...
            if lower_label in {"actual wip", "queue ratio", "move-out", "downstream supply", "q-time"}:
                result["trend_items"].append({"label": label, "value": value, "meta": item.get("meta")})
            elif "effect confirmation status" in lower_label:
                result["effect_confirmation_status"] = value
            elif "recovery evidence" in lower_label:
                result["recovery_evidence"] = value
            elif "risk decision" in lower_label:
                result["risk_decision"] = value
        result["source_flow_no"] = summary.get("flow_no")
        break
    return _drop_empty(result)


def build_flow09_inputs(previous_flow_contents: list[Dict[str, Any]], case_data_snapshot: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        "flow09_inputs": load_flow_mock(),
        "case_data_snapshot": snapshot,
        "sql_results": sql_results,
        "derived_context": {
            "flow08_observation_context": extract_flow08_observation_context(previous_flow_contents),
        },
    }
...
def build_model_context(case_id: str, previous_record: Optional[Any] = None) -> Dict[str, Any]:
    ...
    flow09_context = build_flow09_inputs(previous_flow_contents, case_data_snapshot=case_data_snapshot)
    return {
        "case_id": case_id,
        "flow": {"flow_no": FLOW_NO, "flow_name": FLOW_NAME, "purpose": "影响消除观察"},
        ...
        "generation_rules": [
            ...
            "同时使用 case_data_snapshot.sql_results 中的原始异常事实与 Flow 08 之后 content 中的恢复趋势；不要读取或返回旧 Flow 09 结果，也不得引用 examples 的示例数值。",
            "Flow 09 默认假设影响消除观察通过，因为 Flow 08 的恢复趋势已经作为恢复趋势输入提供。",
            "Flow 09 只观察无复发、无下游转移风险、无处置副作用和临时控制释放准备度；不要直接关闭 Case。",
            "WIP Case Snapshot 仍然只包含两段：Case Header 和 Case Risk Trend｜影响消除观察（稳定趋势）。",
            "正常完成状态 case_status=Processing、next_flow_no=10、next_flow_name=Case 关闭确认。",
            "脚本不构造最终展示结构或固定话术。",
        ],
...
REQUIRED_CONTENT_SHAPE = [
    ("WIP Case Snapshot", ["Case Header", "Case Risk Trend｜影响消除观察（稳定趋势）"]),
    (
        "当前阶段对话",
...
def validate_data_tool_call_items(content: Dict[str, Any]) -> None:
    ...
    if missing:
        raise ValueError("Flow 09 data/tool calls must include impact-clearance dimensions: " + ", ".join(missing))
...
def normalize_model_output(model_output: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "impact_clearance_status": model_output.get("impact_clearance_status"),
    }


def decision_from_model_output(model_output: Dict[str, Any]) -> Dict[str, Any]:
    status = str(model_output.get("impact_clearance_status") or "")
    combined = json.dumps(model_output, ensure_ascii=False)
    blocked_terms = ("复发", "转移风险", "副作用", "证据不足", "未通过", "需回退", "升级", "On Hold", "on hold")
    if any(term in status for term in blocked_terms) and "观察通过" not in status:
        default = {"flow_status": "Closed", "case_status": "On Hold", "next_flow_no": None, "next_flow_name": None}
    elif any(term in combined for term in ("复发未排除", "出现复发", "出现下游转移", "出现处置副作用")):
        default = {"flow_status": "Closed", "case_status": "On Hold", "next_flow_no": None, "next_flow_name": None}
    else:
...
def compose_flow09_result(case_id: str, previous_record: Optional[Any] = None, model_output: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "impact_clearance_status": generated.get("impact_clearance_status"),
        "text": generated["text"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run WIP Bubble Flow 09.")
