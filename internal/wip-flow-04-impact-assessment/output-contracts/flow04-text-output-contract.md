...
Flow 04 只完成“影响范围评估”：在临时措施已生成或已执行后，基于既定业务字段 / SQL 已有字段评估异常对 Impact Lot、Impact WO、Hot Lot / Super Hot Run、Q-Time、Move-Out 和下游供料的影响，判断是否超过单点波动。
...
- `数据 / 工具调用`：必须只使用既定业务字段 / SQL 已有字段，且必须是事实值 + Done 状态，不要只写 Done。
