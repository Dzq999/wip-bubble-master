#!/usr/bin/env python
"""Flow 07: prepare raw inputs for agent-generated collaboration package output."""
...
PROMPT_PATH = Path(__file__).resolve().parents[1] / "prompts" / "flow07_result_prompt.md"
OUTPUT_CONTRACT_DIR = Path(__file__).resolve().parents[1] / "output-contracts"
TEXT_OUTPUT_CONTRACT_PATH = OUTPUT_CONTRACT_DIR / "flow07-text-output-contract.md"
JSON_OUTPUT_CONTRACT_PATH = OUTPUT_CONTRACT_DIR / "flow07-json-output-contract.md"
GLOBAL_KNOWLEDGE_DIR = SKILL_DIR / "knowledge"
FLOW_KNOWLEDGE_DIR = Path(__file__).resolve().parents[1] / "knowledge"
FLOW_MOCK_PATH = Path(__file__).resolve().parents[1] / "data" / "flow07_mock.json"
...
def extract_labeled_values(previous_flow_contents: list[Dict[str, Any]]) -> Dict[str, Any]:
    result: Dict[str, Any] = {"flow05": {}, "flow06": {}}
    for summary in previous_flow_contents:
        flow_no = str(summary.get("flow_no") or "")
        if flow_no not in {"05", "06"}:
            continue
        target = result[f"flow{flow_no}"]
        for item in iter_content_items(summary.get("content", {})):
            ...
            if "case level" in lower_label:
                target["case_level"] = value
            elif "disposition" in lower_label:
                target["disposition_path"] = value
            elif "initial owner" in lower_label or "primary owner" in lower_label:
                target["owner"] = value
            elif "primary candidate" in lower_label:
                target["primary_candidate"] = value
            elif "secondary candidate" in lower_label:
                target["secondary_candidate"] = value
            elif "confidence" in lower_label:
                target["confidence"] = value
            elif "missing evidence" in lower_label:
                target["missing_evidence"] = value
    return _drop_empty(result)


def build_flow07_inputs(previous_flow_contents: list[Dict[str, Any]]) -> Dict[str, Any]:
    return {
        "flow07_inputs": load_flow_mock(),
        "derived_context": extract_labeled_values(previous_flow_contents),
    }
...
def build_model_context(case_id: str, previous_record: Optional[Any] = None) -> Dict[str, Any]:
    ...
    flow07_context = build_flow07_inputs(previous_flow_contents)
    return {
        "case_id": case_id,
        "flow": {"flow_no": FLOW_NO, "flow_name": FLOW_NAME, "purpose": "工程问题包与协同任务"},
        ...
        "generation_rules": [
            ...
            "只从 previous_flows[].content 和 flow07_inputs 获取事实与角色矩阵；不要读取或返回旧 Flow 07 结果。",
            "Flow 07 只生成工程问题包与协同任务建议，不声称已经真实派单、通知、执行或恢复。",
            "任务拆分必须围绕 Flow 06 候选原因、Flow 05 处置路径和 Flow 04 影响范围。",
            "恢复指标必须可在 Flow 08 中验证；不要输出没有验证口径的指标。",
            "脚本不构造最终展示结构或固定话术。",
        ],
...
def validate_data_tool_call_items(content: Dict[str, Any]) -> None:
    ...
    if missing:
        raise ValueError("Flow 07 data/tool calls must include collaboration dimensions: " + ", ".join(missing))
...
def normalize_model_output(model_output: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "collaboration_package_status": model_output.get("collaboration_package_status"),
    }


def decision_from_model_output(model_output: Dict[str, Any]) -> Dict[str, Any]:
    status = str(model_output.get("collaboration_package_status") or "")
    blocked_terms = ("无法生成", "证据不足", "阻塞", "On Hold", "on hold")
...
def compose_flow07_result(case_id: str, previous_record: Optional[Any] = None, model_output: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "collaboration_package_status": generated.get("collaboration_package_status"),
        "text": generated["text"],
...
def run(case_id: str, previous_record: Optional[Any] = None, model_output: Optional[Dict[str, Any]] = None, save: bool = True) -> Dict[str, Any]:
    result = compose_flow07_result(case_id, previous_record=previous_record, model_output=model_output)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run WIP Bubble Flow 07.")
