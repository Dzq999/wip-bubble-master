#!/usr/bin/env python
"""Flow 03: prepare raw inputs for agent-generated containment output."""
...
PROMPT_PATH = Path(__file__).resolve().parents[1] / "prompts" / "flow03_result_prompt.md"
OUTPUT_CONTRACT_DIR = Path(__file__).resolve().parents[1] / "output-contracts"
TEXT_OUTPUT_CONTRACT_PATH = OUTPUT_CONTRACT_DIR / "flow03-text-output-contract.md"
JSON_OUTPUT_CONTRACT_PATH = OUTPUT_CONTRACT_DIR / "flow03-json-output-contract.md"
    ...
    for index, item in enumerate(items):
        ...
        if ":" not in item_text and "：" not in item_text:
            raise ValueError(
                ...
                f"for example 'Hot Lot Query: Hot Lot = 2，Super Hot Run = 3 [Done]': index {index}"
            )
...
def validate_downstream_stage_consistency(content: Dict[str, Any], expected_stage: Optional[str]) -> None:
    ...
    downstream_items = [
        ...
        for item in items
        if "Downstream Risk Check" in stringify_display_item(item) or "下游" in stringify_display_item(item)
    ]
    if not downstream_items:
        raise ValueError("Downstream Risk Check is required when previous Downstream / Next Stage is known")
    mismatched = [item for item in downstream_items if expected_stage not in item]
    if mismatched:
        raise ValueError(
            "Downstream Risk Check must use previous Downstream / Next Stage "
            + repr(expected_stage)
            + ", got: "
            + " | ".join(mismatched)
        )
...
def normalize_model_output(model_output: Dict[str, Any], expected_downstream_stage: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "containment_status": model_output.get("containment_status"),
    }


def decision_from_model_output(model_output: Dict[str, Any]) -> Dict[str, Any]:
    containment_status = str(model_output.get("containment_status") or "")
    blocked_terms = ("无法执行", "未执行", "阻塞", "On Hold", "on hold")
    if any(term in containment_status for term in blocked_terms):
...
def compose_flow03_result(case_id: str, previous_record: Optional[Any] = None, model_output: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "containment_status": generated.get("containment_status"),
        "text": generated["text"],
...
def run(case_id: str, previous_record: Optional[Any] = None, model_output: Optional[Dict[str, Any]] = None, save: bool = True) -> Dict[str, Any]:
    result = compose_flow03_result(case_id, previous_record=previous_record, model_output=model_output)
